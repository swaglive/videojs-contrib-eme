...
const addKey = ({
  ...
  return new Promise((resolve, reject) => {
    if (!video.webkitKeys) {
      try {
        video.webkitSetMediaKeys(new window__default["default"].WebKitMediaKeys(FAIRPLAY_KEY_SYSTEM));
      } catch (error) {
...
const getIsEdgeLegacy = () => {
  return window__default["default"].navigator && window__default["default"].navigator.userAgent && window__default["default"].navigator.userAgent.indexOf('Edge/') > -1;
  ...
  return (emeOptions, keyMessage, contentId) => {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const initDataString = window__default["default"].btoa(String.fromCharCode(...new Uint8Array(initData))).slice(0, 731) || ''; // '731' is to slice `cenc:pssh` part of initData on PlayReady, Widevine pssh length won't exceed 731
...
const onPlayerReady = (player, emeError) => {
  ...
  if (window__default["default"].MediaKeys && !window__default["default"].WebKitMediaKeys) {
